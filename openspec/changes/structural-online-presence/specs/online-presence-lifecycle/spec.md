...
The system SHALL implement structural online presence without changing mux/XUDP/RVS/WireGuard wire bytes, protobuf or user configuration schemas, public routing and stats interfaces, StatsService/CLI metric names, traffic accounting, or unique-IP result semantics.
...
The built-in online map SHALL assign a non-zero generation per map instance and a fresh non-zero token per exact reference. Exact release MUST use only the recorded generation-qualified token. Legacy `AddIP`/`RemoveIP` SHALL affect only legacy refs; visible entry count SHALL use the sum of legacy and exact refs.
...
- **WHEN** one legacy ref and one exact ref share an IP
- **THEN** removing either kind leaves the other visible
- **AND** the entry disappears only after both kinds reach zero
...
The built-in online map MUST validate and replace one or more distinct exact tokens under one lock. A successful replacement MUST consume every old token, create fresh tokens even for the same IP, preserve unrelated refs, and expose only the complete before or after state. Validation failure MUST perform zero mutation.
...
#### Scenario: Custom map fallback preserves availability
- **WHEN** a custom map lacks exact replacement or old/new generations differ
- **THEN** new refs activate before old refs close
- **AND** traffic continues with one rate-limited degraded warning
...
An XUDP rebind SHALL prepare independently while the old attachment remains current. One commit transaction MUST validate flow token, old epoch/attachment token, server reservation, target, and runtime state before an irreversible exact lease handoff and new publication. Precommit failure MUST preserve old state and MUST NOT send first payload; postcommit failure MUST close new state and MUST NOT resurrect old state.
...
- **WHEN** prepare, dispatch, cancellation, target validation, or shutdown fails before commit authorization
...
- **WHEN** first backend enqueue/write fails after handoff authorization
...
- **WHEN** Portal, Bridge, or dynamic VLESS reverse owner closes concurrently with registration, activation, picker work, and controls
- **THEN** admission, handlers, monitors, timers, tasks, workers, registries, runtimes, data leases, and goroutines are drained before Closed is signaled
