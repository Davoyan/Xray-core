...
The trusted presence IP is normally captured at the earliest raw socket `Accept` or packet `ReadFrom` boundary before XFF, protocol, mux-frame, or routing rewrites. Transports preserve this value separately from their effective `RemoteAddr`; `app/proxyman/inbound` freezes it in session metadata. When the existing `acceptProxyProtocol` option is enabled, a successfully parsed PROXY header is the operator's explicit trust decision. The parser/listener seam captures its canonical `Header.SourceAddr` in a separate immutable carrier before HTTP XFF or another virtual transport can rewrite the effective address, and transports preserve both raw and accepted-PROXY provenance independently. A missing, malformed, `LOCAL`, non-IP, or datagram-on-stream PROXY result remains untracked; a successfully parsed source remains valid when it equals the raw peer because parser success removes the previous ambiguity. The provider snapshots the selected value only after authentication has populated the user.
