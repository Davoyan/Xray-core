...
- Release titles, release notes, asset labels, and all other public release
  text must always be written in English.
- Use one canonical stable release and tag for each version. The tag format is
  `vMAJOR.MINOR.PATCH`, and the release title is
  `Xray-core vMAJOR.MINOR.PATCH`. Do not publish temporary, duplicate, or
  fork-suffixed releases when a stable version was requested.
- Before choosing or bumping a version, fetch `upstream/main` and upstream tags.
  If upstream already uses the requested version, merge that upstream release
  first and make the fork release from the combined history. Never overwrite an
  upstream tag in the local tag namespace without first resolving the collision
  deliberately.
- Prepare a release only from the repository's canonical `main` branch. Merge
  the reviewed feature branch and the latest `origin/main` and `upstream/main`
  before tagging. The release tag, remote `main`, and release workflow
  `head_sha` must resolve to the same commit.
- Update `core/core.go` to the requested numeric version and commit the bump
  before creating the tag. Confirm that `core.Version()` reports the intended
  version.
- Run the applicable unit, race, vet, checkptr, interoperability, and Linux
  build gates before publishing. Record real failures and external limitations;
  never hide them with retries or weaken a test for the release.
- Create an annotated tag and push `main` before pushing the tag. Verify both
  remote refs after the push. Do not move or force-update a published release
  tag unless the maintainer explicitly authorizes rewriting it.
- Publish the GitHub Release from the canonical tag and let
  `.github/workflows/release.yml` build and upload the official platform
  matrix. Do not manually upload replacement binaries or ad-hoc archives when
  CI is responsible for release assets.
- Wait for the release workflow to finish. The release is complete only when
  every required matrix job is green and the expected ZIP and `.dgst` assets
  are present. Remove duplicate manual assets only after CI assets are visible.
- Release notes must be substantive, not just links or an autogenerated commit
  list. Use this structure:
