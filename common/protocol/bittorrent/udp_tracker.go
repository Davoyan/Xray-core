...
// may instead open a flow with an announce (action 1, at least 98 bytes,
// event 0..3, nonzero listen port) or a scrape (action 2, 16 + 20N bytes).
func SniffUDPTracker(b []byte) (*SniffHeader, error) {
	...
	if len(b) >= 36 && (len(b)-16)%20 == 0 && binary.BigEndian.Uint32(b[8:12]) == 2 {
		return &SniffHeader{}, nil
	}
