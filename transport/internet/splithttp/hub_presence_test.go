...
func TestSplitHTTPVirtualConnectionsSeparatePhysicalPeerFromXFF(t *testing.T) {
	tests := []struct {
		...
		{name: "H2 connection context", proto: 2, remote: "198.51.100.8:443", wantPeer: "192.0.2.8:8443", withState: true},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.withState {
				...
				source := &splitPresenceConn{Conn: server, remote: &net.TCPAddr{IP: net.ParseIP("192.0.2.8"), Port: 8443}}
